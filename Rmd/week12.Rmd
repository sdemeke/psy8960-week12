...
```{r}
#first noticed some curly quotes. not sure how to handle them in pre-processing so using gsub
#week12_tbl$title <- gsub("’", "'", week12_tbl$title, ignore.case = TRUE)
#use str replace in content_Transformer()
...
io_corpus_original[[17]]$content
io_corpus_original[[120]]$content
io_corpus_original[[40]]$content

my_stopwords <- c("io psychology","io psych","io psychologist")
...
#  tm_map(content_transformer(lemmatize_strings)) %>% 
  ...
  tm_map(content_transformer(replace_contraction)) %>% #not working cuz of curly quotes inside titles, fixed
