...
my_stopwords <- c("io psychology","io psych","iopsychology",
                  "industrial psychology","industrialorganizational psychology","riopsychology","iopsych")
custom_stopwords <- c(stopwords("en"),my_stopwords)

#use str_Replace_all to replace single ’ and double ” curly quotes with normal single quote
#use replace_contraction to lengthen contracted phrases
#after lengthening contractions, remove punctuation marks and numbers
#convert all characters to lower case
#my custom stopwords assume the above preprocessing (e.g., no 'i/o' in my vector) so i remove all stopwords at this point
#final step is to stem by lemmatizing the strings in the corpus

io_corpus <- io_corpus_original %>%
  # tm_map(content_transformer(str_replace_all),pattern = "’|“|”", replacement = "'") %>% 
  # # tm_map(content_transformer(str_replace_all),pattern = "—", replacement = "-") %>% #replace long dash
...
#returns TRUE for all 973, 3 removed
...
# <<DocumentTermMatrix (documents: 975, terms: 5610)>>
# Non-/sparse entries: 8862/5460888
# Sparsity           : 100%
...
#slimmed N:k is 972/455 or 2.13
...
```
My interpretation from `ldatuning` results is that the Deveaud2014 and Arun2010 metrics aren't super informative. The CaoJuan2009 metric suggests 3, 6, and 8 as optimal topic numbers looking at the minima (I also tried a few different random seeds and found similar-ish results). I am  not sure how to interpret results from the Griffiths2004 metric which doesn't seem to have an idenitifiable maximum so I am choosing to continue with the optimal numbers based on the CaoJuan2009 results. Although not shown below, I tried examining beta matrices with 6 and 8 topics and did not find them interpretable so I am choosing to continue with 3 topics.
...
lda_gammas_by_doc <- tidy(io_lda, matrix="gamma") %>%
group_by(document) %>%
top_n(1, gamma) %>%
slice(1) %>%
ungroup %>%
...
#do this better and create new week12 tbl by filtering on tm_index
week12_title_filt <- week12_tbl$title[tm_index(io_corpus, FUN = function(x) { return(nchar(stripWhitespace(x$content)[[1]]) > 0) })] 



topics_tbl <- tibble(
  doc_id = lda_gammas_by_doc$document,
  original = week12_title_filt,
  topic = lda_gammas_by_doc$topic,
  probability = lda_gammas_by_doc$gamma
...
final_tbl <- topics_tbl %>% 
  mutate(upvotes = week12_upv_filt <- week12_tbl$upvote[tm_index(io_corpus, FUN = function(x) { return(nchar(stripWhitespace(x$content)[[1]]) > 0) })] )
```
...
aov_io = aov(upvotes ~ as.factor(topic), data = final_tbl)
summary(aov_io)
```
