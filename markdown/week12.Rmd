...
Finally, ran a statistical regression analysis to determine if upvotes differs by topic. Based on the results, I conclude that the number of votes does significantly differ by topic (overall F statistic is significant at p < 0.05). The coefficients suggest that topic 2, 3, and 4 have significantly more upvotes than topic 1. The coefficient for topic 5 is only marginally significant and not at alpha < 0.05. This may suggest that the posts for topic 1 (reading discussions) get the least upvotes amongst all topics.
...
The wordcloud of `io_dtm` shows the words with the highest frequencies in the provided DTM and is specified to show at most the top 25 most frequent terms. Based on the I/O subreddit DTM, the most frequent words all relate to the topics most common to I/O, including work, job, career, and research. It is exported in the `fig` folder in the current directory as a PNG.

```{r}
color_pal <- viridisLite::inferno(n=6, direction = -1)
